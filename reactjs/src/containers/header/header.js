...
const Header = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/api/v1/listcategory')
      .then(response => {
        setCategories(response.data.categories || []);
      })
      .catch(error => console.error('Error fetching categories:', error));

    axios.get('http://localhost:3000/api/v1/listbrand')
      .then(response => {
          ...
          <div className={`d-flex align-items-center ${styles.actionButtons}`}>
            {user?.auth ? (
              <div className={styles.userDropdown}>
                ...
                >
                  <img
                    src={user.avatar ? `http://localhost:3000/images/useravatar/${user.avatar}` : DFavatar}
                    alt="User Avatar"
